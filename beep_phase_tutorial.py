...
def beep_phase_tutorial(cam, cvNet, classes, target, nn=500, s=0.3, Crop=False, Zoom = False, z = 1.5, show_image = True):
  ...
  flag = 0  #flag used to exit the beep phase
  ...
  time_object_not_located = 10000000
  ...
  while True:
    ...
    if show_image:
      printed_objects = ""
      cv.circle(imgi,(c,r), 5, (102,21,0), -1)
      cv.circle(imgi,(c,r), int(rg[0]*diag/100), (102,21,0))
      cv.circle(imgi,(c,r), int(rg[1]*diag/100), (102,21,0))
      cv.circle(imgi,(c,r), int(rg[2]*diag/100), (102,21,0))
      for obj in objs:
        if obj.is_target:
          colors = (145, 203, 43)
          label1 = "BANANA"
          label2 = "distance = " + str(obj.norm())
          x,y = obj.pos_pix()
          cv.line(imgi, (c, r), (x, y), (251,206,177))
          cv.circle(imgi,(x, y), 5, colors, -1)
          cv.circle(imgi,(x, y), radius, colors, thickness = 2)
          cv.putText(imgi, label1, (x-33, y-radius-15),cv.FONT_HERSHEY_SIMPLEX, 0.5, colors, 2)
          cv.putText(imgi, label2, (x-33, y+radius+15),cv.FONT_HERSHEY_SIMPLEX, 0.5, colors, 2)
        else:
          colors = (0,0,255)
          label1 = "OBSTACLE"
          if not(obj.obj in (printed_objects)):
            x,y = obj.pos_pix()
            cv.line(imgi, (c, r), (x, y), (251,206,177))
            cv.circle(imgi,(x, y), 5, colors, -1)
            cv.circle(imgi,(x, y), radius, colors, thickness = 2)
            cv.putText(imgi, label1, (x-30, y-radius-15),cv.FONT_HERSHEY_SIMPLEX, 0.5, colors, 2)
            printed_objects = printed_objects + obj.obj
    ...
    if flag == 1 and not target_found:
      interval = toc-tic
...
beep_phase_tutorial(cam, cvNet, classes, target)
