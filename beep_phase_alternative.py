import numpy as np
import math
import cv2 as cv # OpenCV computer vision library
from detected_object import DetectedObject
import pygame
import time
import os
from vocal_command import vocal_command
import pyttsx3

#No crop mode only
def beep_phase_alternative(cam, cvNet, classes, target, nn=700, s=0.2, Zoom = True, z = 1.5, show_image = False):
  sounds_path = os.getcwd() + "/sounds/"  #initializations for sounds played
  pygame.mixer.init()
  sounds = ("sound_1", "sound_2", "sound_3", "sound_4")
  playing_sound = None
  toplay_sound = None
  colors = np.random.uniform(0, 150, size=(len(classes), 3)) #initialization of colors print of image
  flag = []  #flag used to exit the beep phase
  z1 = (z-1)/(2*z) #constants used for the zoom
  z2 = (z+1)/(2*z)
  rg= [25, 40, 60] # IMPORTANT!! these are the limits of the ranges used to the define the sound ranges
  norm_ranges = ([0, rg[0]-1], [rg[0], rg[1]-1], [rg[1], rg[2]-1], [rg[2], 100])
  time_object_not_located = 4
  
  #WHILE CICLE OF FRAMES ACQUIRED BY THE CAMERA
  while True:
    ret_val, imgi = cam.read() #read image from camera
    cols = imgi.shape[1] #dir x
    rows = imgi.shape[0] #dir y
    #pre-treatment of image
    if Zoom:
      imgi = imgi[int(rows*z1):int(rows*z2), int(cols*z1):int(cols*z2)]
      cols = imgi.shape[1] #dir x
      rows = imgi.shape[0] #dir y
    c = int(cols/2)  
    r = int(rows/2)
    
    #initialization of variables
    diag = int(math.sqrt(c*c+r*r))
    max_target_score = s
    object_found = False
    target_found = False
    target_really_found = False #in this code there is a double check to see if the target is not really an obstacle
    target_detection = None
    obstacle_found = False
    obstacle_right = False
    obstacle_left = False
    obstacle_detis = [] #only for print of image
    obstacle_norms = []
    
    #No crop - only look at main image
    cvNet.setInput(cv.dnn.blobFromImage(imgi, size=(nn, nn), swapRB=True, crop=False))
    cvOut = cvNet.forward()
    
    #GO THROUGH EACH DETECTED OBJECT AND LABELS IT
    for detection in cvOut[0,0,:,:]:
      score = float(detection[2])
      idx = int(detection[1])
      obj = classes[idx] #name of detection considered
      #if detection is target with higher score found than the previous ones
      if score > max_target_score and obj == target:
        target_found = True
        max_target_score = score
        target_detection = detection
      #if detection is obstacle
      if score > 0.3 and (obj=='banana' or obj=='bottle' or obj=='cell phone' or obj=='toothbrush' or obj=='apple' or obj=='cup') and obj != target:
        obstacle_found = True
        object_found = True
        left = detection[3] * cols
        top = detection[4] * rows
        right = detection[5] * cols
        bottom = detection[6] * rows          
        x_rel = int((right+left)/2)
        y_rel = int((bottom+top)/2)
        obstacle_deti = DetectedObject(0, x_rel, y_rel, obj, target, c, r)
        obstacle_norms.append(obstacle_deti.norm())
        obstacle_detis.append(obstacle_deti) #only for print of image
        if obstacle_deti.x > 0:
          obstacle_right = True
        if obstacle_deti.x < 0:
          obstacle_left = True

    #IMPORTANT: WE CONSIDER THE TARGET TO BE A REAL ONE ONLY IF IT IS RECOGNIZED WITHOUD ANY OTHER OBJECTS
    target_really_found = target_found and not obstacle_found
    if target_really_found:
      object_found = True
    if target_really_found:
      flag.append(1)
      tic = time.perf_counter()
      left = target_detection[3] * cols
      top = target_detection[4] * rows
      right = target_detection[5] * cols
      bottom = target_detection[6] * rows          
      x_rel = int((right+left)/2)
      y_rel = int((bottom+top)/2)
      target_deti = DetectedObject(0, x_rel, y_rel, target, target, c, r)
      target_norm = target_deti.norm()

    #sounds part
    if object_found and not target_really_found:
      obstacle_range = 75
      if z == 2:
        obstacle_range = 100
      if min(obstacle_norms) <= obstacle_range:  
        if obstacle_right and not obstacle_left:
          toplay_sound = sounds_path + "sound_wrongR" + ".mp3"
        elif obstacle_left and not obstacle_right:
          toplay_sound = sounds_path + "sound_wrongL" + ".mp3"
        else:
          toplay_sound = sounds_path + "sound_wrong" + ".mp3"  
      else:
        toplay_sound = None
        playing_sound = None
        
        
    beep = ""
    direction = ""
    up_down = ""
    if target_really_found:
      norm_ranges = ([0, 24], [25, 39], [40, 59], [60,100])
      sounds = ("sound_1", "sound_2", "sound_3", "sound_4")
      for idx_sound, n in enumerate(norm_ranges):
        if (target_norm >= n[0] and target_norm <= n[1]):
          beep = sounds[idx_sound]
      if beep != "sound_1":
        if target_deti.x > 0:
          direction = "R"
        else:
          direction = "L"
        if target_deti.y > int(rg[0]*diag/100): #pixels corresponding to 24 distance
          up_down = "U"
        if target_deti.y < -int(rg[0]*diag/100):
          up_down = "D"
      toplay_sound = sounds_path + beep + direction + up_down + ".mp3"
            
    if not object_found:
      toplay_sound = None
      playing_sound = None
    
    if toplay_sound == None:
      pygame.mixer.music.unload()
    elif toplay_sound != playing_sound:
      pygame.mixer.music.unload()
      pygame.mixer.music.load(toplay_sound)
      pygame.mixer.music.play()
      playing_sound = toplay_sound
    
    #DRAWINGS ON IMAGE
    if show_image:
      cv.circle(imgi,(c,r), 10, (251,206,177), -1)  
      #drawing of target
      if target_really_found:
        x,y = target_deti.pos_pix()
        cv.line(imgi, (c, r), (x, y), (251,206,177))
        cv.circle(imgi,(x, y), 5, (0,0,255), -1)
      for obstacle in obstacle_detis:
        x,y = obstacle.pos_pix()
        cv.line(imgi, (c, r), (x, y), (251,206,177))
        cv.circle(imgi,(x, y), 5, (0,0,255), -1)
      imgi=cv.resize(imgi,(1280,720)) #resize of image to pc format
      cv.imshow('immagine', imgi)
      if cv.waitKey(1) == 27: # Press ESC to quit
          break
  
    # EXIT FROM MODE
    toc = time.perf_counter()
    if len(flag) > 10 and not target_really_found:
      interval = toc-tic
      if interval >= time_object_not_located:
        pygame.mixer.music.unload()
        mytext = "Have you grasped the "+target+"?"
        engine = pyttsx3.init()
        voices = engine.getProperty('voices')
        engine.setProperty('voice', voices[1].id)
        # texting of the speaking 
        engine.say(mytext)
        engine.runAndWait()
        while(True):
          try:
              command = vocal_command()
              print(command)
          except Exception as e:
              print(e)
          if "yes" in command:
              mytext = "Good job!"
              engine = pyttsx3.init()
              voices = engine.getProperty('voices')
              engine.setProperty('voice', voices[1].id)
              # texting of the speaking 
              engine.say(mytext)
              engine.runAndWait()
              return
          if "no" in command:
              flag = []
              mytext = "I try to look again"
              engine = pyttsx3.init()
              voices = engine.getProperty('voices')
              engine.setProperty('voice', voices[1].id)
              # texting of the speaking 
              engine.say(mytext)
              engine.runAndWait()
              break
            
  return